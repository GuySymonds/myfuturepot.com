...
import BuyingPowerExamples from './components/BuyingPowerExamples';
...
export default function App() {
  ...
  // Cost of waiting: difference vs starting at 18, for start age 30
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      {/* 1. Header / hero */}
      <Hero costOfWaiting30={costOfWaiting30} />

      <main className="flex-1 max-w-6xl mx-auto w-full px-4 py-8 space-y-10">

        {/* 2. Cost of Waiting landing section */}
        <WaitingComparison inputs={pensionInputs} />

        {/* 3. "What could that buy today?" cards */}
        <BuyingPowerExamples amountTodayEquivalent={pensionResult.potTodayEquivalent} />

        {/* 4. Collapsible assumptions panel */}
        <AssumptionsPanel inputs={pensionInputs} onChange={setPensionInputs} />

        {/* 5. Pot through the years chart */}
        <Card>
        ...
        {/* 6. Result breakdown cards */}
        <div className="flex flex-col gap-6">
          <div className="flex items-center gap-2">
            <h2 className="text-2xl font-bold text-slate-800">Your pension estimate</h2>
            <HelpTip text="Based on the assumptions set above. These are estimates for illustration only — not a guaranteed outcome." />
          </div>
        ...
        {/* 9. Historical examples placeholder */}
        <Card>
          <h2 className="text-lg font-semibold text-slate-700 mb-2">Historical examples</h2>
          <p className="text-sm text-slate-500 leading-relaxed">
            Later, this section could show examples based on historical market and UK price data.
            Historic performance can help explain the past, but it does not predict future returns.
          </p>
        </Card>
